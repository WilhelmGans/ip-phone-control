...
if($numberPhoneGETError == ""){
	...
	if($numberPhoneGETError == ""){
		...
		if ($_GET['receiver'] == "up"){
			...
			//$dbphones->query("INSERT INTO `actions` (`id`, `type`, `user`, `user_level`, `theme`, `text`, `subject`,`date`) VALUES (NULL, 'setting phone', 'Phone', 'admin', 'Успешная авторизации телефона', 'Телефон c номером: $numberPhoneGET <br> mac: $macGET; <br> ip: $ipGET', '$numberPhoneGET', '$today')");
			$dbphones->query("UPDATE `phones` SET `timestamp` = '1', `check_ping` = '0', `ip` = '$ipGET', `mac` = '$macGET' WHERE `ip` = '$ipGET'");
			insertAction(0, 'setting phone', 'Phone', 'admin', 'Ошибка телефон не найден в базе',"Телефон c номером: $numberPhoneGET <br> mac: $macGET; <br> ip: $ipGET <br>", $linkRegistration, $numberPhoneGET, $today);
    }
	}
}else{
	...
	if ($_GET['receiver'] == "up"){
		...
		//$dbphones->query("INSERT INTO `actions` (`id`, `type`, `user`, `user_level`, `theme`, `text`, `subject`,`date`) VALUES (NULL, 'setting phone', 'Phone', 'admin', 'Успешная авторизации телефона', 'Телефон c номером: $numberPhoneGET <br> mac: $macGET; <br> ip: $ipGET', '$numberPhoneGET', '$today')");
		$dbphones->query("UPDATE `phones` SET `timestamp` = '1', `check_ping` = '0', `ip` = '$ipGET', `mac` = '$macGET' WHERE `number` = '$numberPhoneGET'");
				insertAction(0, 'setting phone', 'Phone', 'admin', 'Ошибка телефон не найден в базе',"Телефон c номером: $numberPhoneGET <br> mac: $macGET; <br> ip: $ipGET <br>", $linkRegistration, $numberPhoneGET, $today);
		}
}
echo $rec;
//https://192.168.44.2/scriptPHP/phoneConnectAndUpdate/phoneRegister.php?ip=$ip&mac=$mac&numberPhone=$active_user&receiver=up
//https://192.168.44.2/scriptPHP/phoneConnectAndUpdate/phoneRegister.php?ip=$ip&mac=$mac&numberPhone=$active_user&receiver=down
?>